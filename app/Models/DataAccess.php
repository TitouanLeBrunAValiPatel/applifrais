...
// TODO : Transformer toutes les requêtes en requêtes paramétrées
     ...
     * @param $mois sous la forme aaaamm
     */
    public function signeFiche($id, $mois) {
        // met à 'CL' son champs idEtat
        $laFiche = $this->getLesInfosFicheFrais($id, $mois);
        if ($laFiche['idEtat'] == 'CL') {
            $this->majEtatFicheFrais($id, $mois, 'VA');
        }
    }

    public function refuseFiche($id, $mois, $comment) {
        // met à 'CL' son champs idEtat
        $laFiche = $this->getLesInfosFicheFrais($id, $mois);
        if ($laFiche['idEtat'] == 'CL') {
            $this->majEtatFicheFrais($id, $mois, 'RE', $comment);
        }
    }
     ...
     */
    public function majEtatFicheFrais($idVisiteur, $mois, $etat, $comment = "") {
        $req = "update ficheFrais 
				set idEtat = '$etat', dateModif = now(), comment = '$comment' 
				where fichefrais.idvisiteur ='$idVisiteur' and fichefrais.mois = '$mois'";
        $this->db->simpleQuery($req);
    }
    ...
    public function getLeStatut($idStatut) {

        $req = "SELECT statut FROM `visiteur` WHERE id = '$idStatut' ;";
        $rs = $this->db->query($req);
        $laLigne = $rs->getFirstRow('array');
        $leStatut = $laLigne['statut'];
        return $leStatut;
    }

    public function getLesEtats() {
        $req = "SELECT id, libelle FROM `etat` WHERE id != 'CR'";
        $rs = $this->db->query($req);
        $lesEtats = $rs->getResultArray();
        return $lesEtats;
    }
